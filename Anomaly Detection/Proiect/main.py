...
class Isolation_Tree:
    def __init__(self, X, max_height, height=0):
        ...
        if height >= max_height or X.shape[0] <= 1:
            ...
            min_feature = np.min(X[:, self.feature])
            max_feature = np.max(X[:, self.feature])
            self.split_point = np.random.uniform(min_feature, max_feature)
            self.left_tree = Isolation_Tree(X[X[:, self.feature] < self.split_point], self.max_height, self.height + 1)
            self.right_tree = Isolation_Tree(X[X[:, self.feature] >= self.split_point], self.max_height,
                                             self.height + 1)
    ...
    def path_length(self, data, length=0):
        ...
        if data[self.feature] < self.split_point:
...
class Isolation_Forest:
    ...
    def fit(self, X):
        ...
        for i in range(self.tree_number):
            ...
            isolation_tree = Isolation_Tree(sub_sample, self.max_height)
...
# ceva = Isolation_Forest(1000, contamination=0.1)
# ceva.fit(X)
...
# data = np.loadtxt("C:/Users/Andrei/Downloads/ionosphere.txt", delimiter=",")
data = np.loadtxt("C:/Users/Andrei/Downloads/cover.txt", delimiter=",")
...
cont = np.mean(y)
ceva = Isolation_Forest(1000, contamination=cont, sub_sample_size=128)
ceva.fit(X_train)


score1 = ceva.anomaly_score(X_test)
precision, recall, thresholds = precision_recall_curve(y_test, score1)
